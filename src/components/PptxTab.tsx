    ...
    <div className="space-y-6 animate-fade-in" id="pptx-tab">
      ...
      <div className="glass-panel rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-navy">لوحة توليد العروض والمقترحات للشركات (AI-to-PPTX)</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <form onSubmit={handleGenerateDeck} className="bg-white border border-navy/5 rounded-2xl p-5 shadow-sm space-y-5">
            <div className="space-y-2">
              <label className="block text-sm font-bold text-navy">موضوع العرض التقديمي وتفاصيل المحتوى:</label>
              <textarea
                value={topicPrompt}
                ...
                required
                disabled={isLoading}
                className="w-full px-4 py-3 rounded-xl border border-navy/10 placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-gold focus:border-transparent text-navy disabled:bg-slate-50 leading-relaxed"
              />
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              <button
                ...
                className="flex-1 py-3 bg-navy hover:bg-navy-light text-gold hover:text-white font-semibold rounded-xl text-xs transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer disabled:bg-slate-100 disabled:text-slate-400"
              >
                {isLoading ? (
                  <>
          ...
          {/* Model tracker detail */}
          <div className="bg-surface rounded-2xl p-4 border border-navy/5 text-xs text-slate-500 leading-relaxed space-y-2">
            ...
            <p>1. نأمر محرك <strong>Claude Sonnet 4.6</strong> بتحليل الفكرة وهندسة مصفوفة JSON متكاملة تمثل شرائح متتالية باللغة العربية الفصحى.</p>
            <p>2. يتم تمشيط تلك الكتل برمجياً وحقنها في محركات <strong>PptxGenJS</strong> لبناء الشرائح مع تمثيل مريح للألوان والمحاذاة RTL.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-3">
          {isLoading ? (
            <div className="bg-white border border-navy/5 rounded-2xl p-8 h-full flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400 max-w-sm">يقوم معالج Claude 4.6 ببناء المحتوى في هذه الأثناء. يرجى إبقاء هذه الصفحة مفتوحة.</p>
              </div>
              <div className="w-full max-w-md p-3 bg-slate-50 rounded-xl max-h-[160px] overflow-y-auto text-left font-mono text-[10px] text-slate-500" dir="ltr">
            ...
            <div className="space-y-4 animate-slide-up">
              ...
              <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="px-5 py-3 bg-navy hover:bg-navy-light text-gold hover:text-white font-bold text-xs rounded-xl shadow transition-all flex items-center gap-2 cursor-pointer self-stretch sm:self-auto justify-center"
                >
                  {downloadSuccess ? (
                    <>
              ...
              <div className="space-y-4">
                {generatedSlides.map((slide, sIdx) => (
                  <div key={sIdx} className="bg-[#FDFBF7] border border-gold/15 hover:border-gold/30 rounded-2xl p-5 shadow-sm relative overflow-hidden transition-all">
                    {/* Index header */}
                    <div className="absolute right-0 top-0 text-[10px] font-mono font-bold text-gold-hover bg-gold/10 px-3 py-1 rounded-bl-xl">
                      شريحة {sIdx + 1}
                    </div>

                    <div className="space-y-3 mt-2 pr-2">
                      <div className="space-y-1">
                        <span className="text-[10px] text-slate-400 uppercase tracking-widest block font-bold">العنوان الأساسي للشرية / Slide Title</span>
                        <h4 className="text-base font-bold text-navy leading-relaxed">{slide.title}</h4>
                      </div>

                      {slide.subtitle && (
                        <div className="space-y-0.5">
                          <span className="text-[10px] text-slate-400 block font-bold">العنوان الفرعي للملخص / Subtitle</span>
                          <p className="text-xs text-gold-hover font-semibold">{slide.subtitle}</p>
                        </div>
                      )}
            ...
            <div className="bg-white border border-navy/5 rounded-2xl p-12 h-full flex flex-col items-center justify-center text-center text-slate-400 space-y-4">
              <HelpCircle className="w-12 h-12 text-gold animate-pulse" />
              <div>
